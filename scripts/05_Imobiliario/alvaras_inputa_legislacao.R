...
sisacoe <- read_csv2(here("inputs", 
                          "1_inbound","GestaoUrbana", "SISACOE",
                          "msp_empreendimentos_sisacoe.csv")) %>%
  transmute(
    alvara_data_emissao = lubridate::dmy(alvara_data_emissao),
    alvara_numero, sql,
...
alvaras_por_lote_legislacao <- read_xlsx(here("inputs", 
                                              "1_inbound", "Insper", "Licenciamentos", "classificacao_legislacao",
                                              "alvaras_por_lote_inputa_legislacao_quali.xlsx")) %>%
...
tipologia_zona_de_uso <- readxl::read_excel(here("inputs",
  ...
  arrange(hierarquia) %>%
  transmute(
    zona_de_uso = str_squish(zona_de_uso) %>% 
      str_replace_all("​", "") %>%
      str_replace_all("X", ".") %>%
...
modelo_zona_de_uso <- tipologia_zona_de_uso %>%
  ...
  rowwise() %>%
  mutate(
    legislacao_zona_de_uso = str_c(pde_zona_de_uso, lpuos_zona_de_uso, sep = "e"),
...
tipologia_zona_de_uso_anterior <- readxl::read_excel(here("inputs",
  ...
  arrange(hierarquia) %>%
  transmute(
    zona_de_uso_anterior = str_squish(zona_de_uso_anterior) %>%
      str_replace_all("​", "") %>%
...
modelo_zona_de_uso_anterior <- tipologia_zona_de_uso_anterior %>%
  ...
  rowwise() %>%
  mutate(
    legislacao_zona_de_uso_anterior = str_c(pde_zona_de_uso_anterior, 
...
modelo_zoneamentos <- tipologia_zoneamentos %>%
  ...
  rowwise() %>%
  mutate(
    legislacao_zoneamentos = str_c(pde_zoneamentos, lpuos_zoneamentos, sep = "e"),
...
# Conjunto enviado = objeto "nao_classificados_alvo"
#
writexl::write_xlsx(list("sem_classificacao" = nao_classificados_alvo,
                         "divergentes" = divergentes),
                    here("inputs",
                         "1_inbound", "Insper",
                         "Licenciamentos", "classificacao_legislacao",
                         "alvaras_por_lote_sem_legislacao.xlsx"))

# Classifica casos sem atribuição de legislação
